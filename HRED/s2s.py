...
class SourceEncoder(nn.Module):
	...
	def forward(self, src, src_len):
		...
		#initial decoder hidden is final hidden state of the forwards and backwards
		#encoder RNNs fed through a linear layer
		hf = torch.cat((hidden[-2,:,:], hidden[-1,:,:]), dim = 1)
		#hf = torch.cat((hfT, hfI), dim = 1)
		hidden = self.fc(hf)
		hidden = self.relu(hidden)
		#outputs = [src len, batch size, enc hid dim * 2]
...
class Decoder(nn.Module):
	...
	def forward(self, input, hidden, enc_hidden, encoder_outputs, mask):
		...
		rnn_input = torch.cat((embedded, weighted, enc_hidden), dim = 2)
...
class classifierNet(nn.Module):
	def __init__(self, ipSize):
		super().__init__()
		self.fc1 = nn.Linear(ipSize, 100)
		self.relu1 = nn.ReLU()
...
class Seq2Seq(nn.Module):
	def __init__(self, classif, srcEncoder, encoder, decoder, src_pad_idx, device):
		super().__init__()
		self.encoder = encoder
		self.decoder = decoder
		self.srcEncoder = srcEncoder
		self.classif = classif
		self.context = nn.GRU(self.encoder.dec_hid_dim, self.encoder.dec_hid_dim)
		self.src_pad_idx = src_pad_idx
		self.device = device
	def create_mask(self, src):
		mask = (src != self.src_pad_idx).permute(1, 0)
		return mask
	def forward(self, src, src_len, ttl, ing, ttl_len, ing_len, trg, teacher_forcing_ratio = 0.5):
		#src = [src len, batch size]
		#src_len = [batch size]
		...
		#decHidden = torch.zeros(trg_len, batch_size, self.encoder.dec_hid_dim).to(self.device)
		#encoder_outputs is all hidden states of the input sequence, back and forwards
		#hidden is the final forward and backward hidden states, passed through a linear layer
		#ttl, ing, ttl_len, ing_len
		#(self, ttl, ing, ttl_len, ing_len)
		encoder_outputs, _ = self.encoder(ttl, ing, ttl_len, ing_len)
		hidden = self.srcEncoder(src, src_len)
		_, enc_hidden = self.context(hidden.unsqueeze(0))
		...
		isEnd = self.classif(hidden.squeeze(0))
		return outputs, isEnd.squeeze(1)