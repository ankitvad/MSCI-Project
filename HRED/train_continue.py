...
#src, ttl, ing, TTL_ING, INS, model, device, max_len
def translate_sentence(src, ttl, ing, src_field, trg_field, model, device, max_len = 15):
	model.eval()
	'''
	tokens = [token.lower() for token in sentence]
	tokens = [src_field.init_token] + tokens + [src_field.eos_token]
	...
	src_tensor, src_len = translateData(src, trg_field)
	with torch.no_grad():
		#hidden = self.encoder(ttl, ing, ttl_len, ing_len)
		encoder_outputs, _ = model.encoder(ttl_tensor, ing_tensor, ttl_len, ing_len)
		hidden = model.srcEncoder(src_tensor, src_len)
		_, enc_hidden = model.context(hidden.unsqueeze(0))
...
def testOP(data, index, tp):
	...
	trg = vars(data.examples[index])['trg']
	src = vars(data.examples[index])['src']
	translation = translate_sentence(src, ttl, ing, TTL_ING, INS, model, device)
	print(f'ttl = {ttl}')
	print(f'ing = {ing}')
	print(f'trg = {trg}')
...
def calculate_bleu(data, src_field, trg_field, model, device, max_len = 15):
	...
	for datum in tqdm(data):
		...
		trg = vars(datum)['trg']
		src = vars(datum)['src']
		#translate_sentence(src, ttl, ing, src_field, trg_field, model, device, max_len = 15)
		pred_trg = translate_sentence(src, ttl, ing, src_field, trg_field, model, device, max_len)
		#cut off <eos> token
		pred_trg = pred_trg[:-1]
...
def train(model, iterator, optimizer, criterion, labelcriterion, clip):
	model.train()
	epoch_loss = 0
	endWeight = 0.5
	...
	for i, batch in enumerate(iterator):
		...
		if i%100 == 0:
			...
			batchStartTime = time.time()
			#tv_datafields = [("title", TTL_ING), ("ing", TTL_ING), ("src", INS), ("trg", INS), ("last", LABEL)]
		...
		output, iE = model(src, src_len, ttl, ing, ttl_len, ing_len, trg)
		#print()
		...
		loss2 = labelcriterion(iE, isend.type_as(iE))
		loss = loss1 + (endWeight * loss2)
		loss.backward()
...
def evaluate(model, iterator, criterion):
	...
	with torch.no_grad():
		...
		output, iE = model(src, src_len, ttl, ing, ttl_len, ing_len, trg)
			...
			src, src_len = batch.src
			isend = batch.last
			output, iE = model(src, src_len, ttl, ing, ttl_len, ing_len, trg, 0.0)
...
if __name__ == '__main__':
	...
	#Title   Ingredients     Source  Target  Last
	LABEL = Field(sequential = False, use_vocab = False)
	TTL_ING = Field(tokenize = "spacy", tokenizer_language="en", init_token='<sos>', eos_token='<eos>', lower = True, include_lengths = True)
	INS = Field(tokenize = "spacy", tokenizer_language="en", init_token='<sos>', eos_token='<eos>', lower = True, include_lengths = True)
	#title,ing,ins
	tv_datafields = [("title", TTL_ING), ("ing", TTL_ING), ("src", INS), ("trg", INS), ("last", LABEL)]
	device = torch.device('cuda:2' if torch.cuda.is_available() else 'cpu')
	trn, vld, tst = TabularDataset.splits(path = "../data", train = 'train_separated.tsv', validation="valid_separated.tsv", test = "test_separated.tsv", format = 'tsv', skip_header = True, fields = tv_datafields)
	...
	#(self, input_dim, emb_dim, enc_hid_dim, dec_hid_dim, dropout, emb, use_emb = True)
	srcEnc = SourceEncoder(OUTPUT_DIM, DEC_EMB_DIM, DEC_HID_DIM, DEC_HID_DIM, DEC_DROPOUT, INS.vocab.vectors, use_emb = True)
	...
	classif = classifierNet(DEC_HID_DIM)
	#(self, encoder, decoder, src_pad_idx, device)
	model = Seq2Seq(classif, srcEnc, enc, dec, SRC_PAD_IDX, device).to(device)
	best_valid_loss = float('inf')
	model.apply(init_weights)
	optimizer = optim.Adam(model.parameters(), lr = LR)
	TRG_PAD_IDX = INS.vocab.stoi[INS.pad_token]
	criterion = nn.CrossEntropyLoss(ignore_index = TRG_PAD_IDX)
	labelcriterion = nn.BCEWithLogitsLoss()
	x = open("result.txt","a")
	...
	if mType == "train":
		for epoch in range(N_EPOCHS):
			...
			train_loss = train(model, train_iterator, optimizer, criterion, labelcriterion, CLIP)
		...
		for i in ep:
			getResults(model, test_iterator, criterion, tst, vld, trn, mName = 'tut-model-epoch'+str(i)+'.pt')
	elif mType == "continue":
		nE = sys.argv[2]
