...
def translate_sentence(ttl, ing, src_field, trg_field, model, device, max_len = 150):
	...
	with torch.no_grad():
		...
		hidden = model.encoder(ttl_tensor, ing_tensor, ttl_len, ing_len)
	...
	for i in range(max_len):
		...
		with torch.no_grad():
			#forward(self, input, hidden, enc_hidden)
...
if __name__ == '__main__':
	...
	train_iterator, valid_iterator, test_iterator = BucketIterator.splits((trn, vld, tst), batch_size = BATCH_SIZE, sort_within_batch = True, sort_key = lambda x : len(x.title), device = device)
	...
	dec = Decoder(OUTPUT_DIM, DEC_EMB_DIM, DEC_HID_DIM, DEC_DROPOUT, INS.vocab.vectors, use_emb = True)
